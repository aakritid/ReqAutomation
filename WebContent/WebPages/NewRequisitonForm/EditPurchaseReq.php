  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div  class="col-sm-5 form-inline form-group pull-right">
            ...
            <select class="form-control required" name="JobCode" id="ddown" onchange="jobCodeChange(this.value)">
				...
				<?php
					...
					while ($row = $result->fetch_assoc()) {
						if($jcd==$row['jobcode'])
							echo "<option selected='selected' value='" . $row['jobcode'] . "'>" . $row['jobcode'] . "</option>";
						else
		...
		<div class="pull-right" id="jcBudg"><label>Remaining Budget: $<?php echo $rembg; ?></label></div>
			     ...
			     <tbody>
			      <tr>
							...
							<?php
								...
								while ($row = $result->fetch_assoc()) {
									if($vendor['VendorName']==$row['VendorName'])
											echo "<option selected='selected' value='" . $row['VendorName'] . "'>" . $row['VendorName'] . "</option>";
									else
							...
							<?php
								...
								while ($row = $result->fetch_assoc()) {
									if($ship['AddrId']== $row['AddrId'])
										echo "<option selected='selected' value='" . $row['AddrId'] . "'>" . $row['Name'] . "</option>";
									else
			        ...
			        <textarea class="form-control required" rows="5" id="shipAddress" name="shipAddr" id="shipAdd"><?php echo sprintf("%s,\n%s,\n%s,\n%s-%s.\n%s.", $addr['Name'], $addr['Address'], $addr['City'], $addr['State'], $addr['ZipCode'], $addr['Country']) ; ?></textarea> 
					</div>
					</td>
			        <td class="col-sm-4"> <div class="checkbox"><label class="checkbox-inline"><input type="checkbox" value="" name="bugd" id="budgt" <?php if ($prdts['Budgeted']==0){?> checked <?php }?> >Budgeted</label>
						</div> 
						<div id="budgdiv"><label for="bcs">BCS#:</label><input  type="text" class="form-control digit" id="bcs" name="bcs" value='<?php echo $prdts['BCS']; ?>'/></div></td>
			      </tr>
			      <tr>
			        <td id="vaddrdiv" colspan="2" class="col-sm-4"><label for="vendorAddress">Address of Vendor:<span class="reqd">*</span></label> 
			        <textarea class="form-control required" rows="4" id="vendorAddress" name="vendAddr"><?php echo $vendor['VendorAddress']; ?></textarea> </td>
			        <td id="attndiv" class="col-sm-4"><label for="attn">Attention:<span class="reqd">*</span></label> 
			        <input type="text" class="form-control required" id="attn" name="attn" value='<?php echo $ship['Attn']; ?>'/></td>
			        <td  class="col-sm-4"> <div class="checkbox"><label class="checkbox-inline"><input type="checkbox" value="" name="nbug" id="nbudgt" <?php if ($prdts['Budgeted']==1){?> checked <?php }?> >Non-Budgeted</label>
						</div> 
						<div id="nbudgtdiv"><label for="explain">Explanation:</label><input type="text" class="form-control" id="explain" name="explain" value='<?php echo $prdts['Expl']; ?>'/></div></td>
			      </tr>
			      <tr>
    ...
    <input type="text" class="form-control required" id="datepicker" name="daten" value='<?php echo $ship['Date']; ?>'></div></td>
			         ...
			         <label><input type="radio" name="scope" value="pind" <?php if ($prdts['Scope']==0){?> checked <?php }?> >PARI India Scope</label>
						</div> 
						<div class="row-sm-2 radio"><label><input type="radio" name="scope" value="pinc" <?php if ($prdts['Scope']==1){?> checked <?php }?>>PARI Inc Scope</label>
						</div>
						<div class="row-sm-2 radio"><label><input type="radio" name="scope" value="other" <?php if ($prdts['Scope']==2){?> checked <?php }?>>Other</label><input type="text" class="form-control" name="otherval" value='<?php echo $prdts['Other']; ?>' />
						</div>
					 </td>
				</tr>
			      <tr>
					...
					<div id="smdiv" class="form-inline selectContainer">
						<select class="form-control required" name="shipMethod" id="shipMethod">
							...
							<option <?php if ($ship['Method']=="FedEx"){?> selected='selected' <?php }?> value="FedEx">FedEx</option>
							<option <?php if ($ship['Method']=="Freight"){?> selected='selected' <?php }?> value="Freight">Freight</option>
							<option <?php if ($ship['Method']=="UPS"){?> selected='selected' <?php }?> value="UPS">UPS</option>
							<option <?php if ($ship['Method']=="USPS"){?> selected='selected' <?php }?> value="USPS">USPS</option>
							<option <?php if ($ship['Method']=="UTI"){?> selected='selected' <?php }?> value="UTI">UTI</option>
							<option <?php if ($ship['Method']=="Your Truck"){?> selected='selected' <?php }?> value="Your Truck">Your Truck</option>
						</select>
					</div>
