...
<link rel="stylesheet" href="http://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css">
  ...
  $(document).ready(function(){
     ...
     $("#addItem").click(function(){
		 i++; 
      $('#itemDesc'+i).html( "<td><input type='text' name='itemNo"+i+"'  placeholder='Item #' class='form-control'/> </td><td><textarea name='item"+i+"'  rows='2' placeholder='Item Description' class='form-control'></textarea></td> <td><input type='text' name='quant"+i+"' id='quant"+i+"' placeholder='Quantity' class='form-control' onchange='totalCalc()'/></td>" +
    		  "<td><input type='text' name='unit"+i+"'  placeholder='Unit' class='form-control'/></td> <td><input type='text' name='unitPrice"+i+"' id='unitPrice"+i+"' placeholder='Unit Price' class='form-control' onchange='totalCalc()'/></td>" +
  ...
  function totalCalc(){
    ...
    document.getElementById("total"+i).value = "$" + x*y;
	...
	<div class="container">
      ...
      <table class="table table-bordered" style="padding:10px" >
		...
		<tbody id="itemTable">
			...
			<td>
				<input type="text" name='quant0'  placeholder='Quantity' id="quant0" class="form-control" onchange="totalCalc()"/>
			</td>
			<td>
				<input type="text" name='unit0'  placeholder='Unit' class="form-control"/>
			</td>
			<td>
				<input type="text" name='unitPrice0'  placeholder='Price' id="unitPrice0" class="form-control" onchange="totalCalc()"/>
			</td>
			<td>
		...
		<div class="form-inline col-sm-8 pull-left"><label for="refQuote">Reference Quote:</label><input class="form-control" name="refQuote" type="text" placeholder="Reference Quote" id="refQuote"/></div>
			 <div class="col-sm-3 pull-right"><label for="totalCost">Total Cost: $</label><label id="totalCost">0</label></div>
